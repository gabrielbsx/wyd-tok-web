                    ...
                    <div class="col-span-4 grid grid-cols-8 my-1 px-1 py-4 px-6 bg-gray-800 border-b-4 border-gray-900 text-gray-300 rounded-lg bg-black">
                        <div class="col-span-4 px-1 py-2">
                            <span class="text-left">
                                <span class="text-left">
                                    R$ <?= $value['picpay_value'] ?? $value['mercadopago_value'] ?>,00
                                </span>
                            </span>
                        </div>
                        <div class="col-span-2 px-1 py-2">
                            <span class="text-left">
                                <?= ($value['picpay_createdAt'] ?? $value['picpay_updatedAt']) ?? ($value['mercadopago_createdAt'] ?? $value['mercadopago_updatedAt']) ?>
                            </span>
                        </div>
                        <div class="col-span-2 py-2 text-right">
                            <?php if (!($value['picpay_status'] || $value['mercadopago_status'])) : ?>
                                <a target="_blank" href="<?= $value['picpay_paymentUrl'] ?? $value['mercadopago_paymentUrl'] ?>" class="col-span-1 px-12 mr-2 py-2 rounded-lg border-b-4 bg-green-700 hover:bg-green-600 hover:border-green-800 border-green-900">
                                    Pagar
                                </a>
                            <?php endif; ?>
                            <span class="col-span-1 px-12 py-2 rounded-lg border-b-4 <?= $value['picpay_status'] || $value['mercadopago_status'] ? 'bg-green-700 hover:bg-green-600 hover:border-green-800 border-green-900' : 'bg-yellow-700 hover:bg-yellow-600 hover:border-yellow-800 border-yellow-900' ?>">
                                <?= $value['picpay_status'] || $value['mercadopago_status'] ? 'Pago' : 'Pendente' ?>
                            </span>
                        </div>
                    </div>
                <?php endforeach; ?>
